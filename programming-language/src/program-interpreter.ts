...
import {
    ...
    getMatrixRowLength,
    getSliceValue,
    isIndexInSliceBounds,
    Matrix,
    matrixAddElements,
    matrixDivideElements,
    matrixElementsEqual,
    matrixElementsGreaterThan,
    matrixElementsGreaterThanOrEqual,
    matrixElementsLessThan,
    matrixElementsLessThanOrEqual,
    matrixLogicalAndElements,
    matrixLogicalOrElements,
    matrixMultiplyElements,
    matrixShapesAreEqual,
    matrixSubtractElements,
    matrixZeroes,
    newSlice,
    setSliceValue,
    subMatrixShapeEqualsRowShape
} from "src/utils/matrix-math";
import { BuiltinFunction, getBuiltinFunctionsMap } from "./program-interpreter-builtins";
import {
    BIN_OP_ADD, BIN_OP_AND_AND, BIN_OP_DIVIDE, BIN_OP_GREATER_THAN, BIN_OP_GREATER_THAN_EQ, BIN_OP_INVALID, BIN_OP_IS_EQUAL_TO, BIN_OP_LESS_THAN, BIN_OP_LESS_THAN_EQ,
    BIN_OP_MODULO,
    BIN_OP_MULTIPLY, BIN_OP_OR_OR, BIN_OP_SUBTRACT,
    BinaryOperatorType,
    binOpToOpString,
    binOpToString,
    DiagnosticInfo,
    ...
    ProgramParseResult,
    T_ASSIGNMENT, T_BINARY_OP, T_BLOCK, T_DATA_INDEX_OP, T_FN, T_IDENTIFIER, T_IDENTIFIER_THE_RESULT_FROM_ABOVE, T_LIST_LITERAL,
    T_MAP_LITERAL,
    T_NUMBER_LITERAL, T_RANGE_FOR, T_STRING_LITERAL, T_TERNARY_IF, T_UNARY_OP, T_VECTOR_LITERAL,
    TextPosition,
    UNARY_OP_NOT,
    UNARY_OP_PRINT,
    UnaryOperatorType,
...
import { CssColor } from "./utils/colour";
import { newRandomNumberGenerator, RandomNumberGenerator, setRngSeed } from "./utils/random";
...
export function addError(result: ProgramInterpretResult, step: ProgramExecutionStep, problem: string, betterPos: TextPosition | null = null) {
...
export function newEmptyProgramOutputs(): ProgramOutputs {
    return {
        prints: [],
...
const builtinFunctions = getBuiltinFunctionsMap();

export function getOrAddNewPlot(result: ProgramInterpretResult, idx: number): ProgramPlotOutput {
...
export function stepProgram(program: ProgramInterpretResult): boolean {
    ...
    switch(step.type) {
        ...
        case EX_STEP_MAP: {
            ...
            const map: ProgramResult = { t: T_RESULT_MAP, map: new Map(), keys: [] };
            for (let i = 0; i < step.n; i++) {
                ...
                if (!map.map.has(key)) {
                    ...
                    addError(program, step, `Duplicate key: ${key}`);
                    return false;
                }
            }
